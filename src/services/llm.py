...
# --- Relative Imports (PROXY_POOL aktif lagi) ---
from ..config import (
    GEMINI_API_KEYS, GROQ_API_KEYS, COHERE_API_KEYS, REPLICATE_API_KEYS,
    HF_API_TOKENS, OPENROUTER_API_KEYS, MISTRAL_API_KEYS,
    PROXY_POOL, # Pastikan PROXY_POOL di-import
    reload_proxy_pool # Import fungsi reload
)
...
# --- Helper Function for Proxy (Tidak berubah) ---
...
def clean_ai_response(raw_text: str) -> str:
    ...
    if text.startswith("```") and text.endswith("```"):
        ...
        if len(lines) > 1: cleaned_text = '\n'.join(lines[1:-1]).strip()
            ...
            try:
                first_space = text.index(' '); last_ticks = text.rindex('```')
                if first_space < last_ticks: cleaned_text = text[first_space + 1:last_ticks].strip()
                else: cleaned_text = text[3:-3].strip()
...
# ============================================================
# CORE AI CALLER (MANUAL FALLBACK + MARK FAILED PROXY)
# ============================================================
def call_llm(prompt: str) -> Optional[str]:
    """Panggil AI menggunakan fallback manual & tandai proxy gagal."""
    if not llm_call_options:
        logger.error("LLM call options not initialized."); return None

    # Acak ulang urutan setiap kali dipanggil, biar nggak stuck di awal terus
    ...
    for i, option in enumerate(current_options):
        ...
        proxy_used = params.get("proxy") # Ambil proxy yg akan dipakai
        ...
        # === PERBAIKAN DI SINI ===
        # Tangkap error koneksi/timeout -> tandai proxy gagal
            ...
            if PROXY_POOL and proxy_used:
                PROXY_POOL.mark_failed(proxy_used) # Tandai proxy gagal
            continue # Lanjut ke opsi berikutnya
        # Tangkap error API lain (Auth, Rate Limit, Bad Request, Not Found)
        except (AuthenticationError, BadRequestError, RateLimitError, NotFoundError) as e:
            logger.warning(f"❌ API Error with {provider} - {model_id}: {type(e).__name__} - {str(e)[:150]}. Trying next...")
            # Jika Rate Limit, mungkin perlu logic cooldown API Key? (Belum ada)
            continue
        # Tangkap Exception umum
            ...
            if PROXY_POOL and proxy_used: # Tandai juga jika error aneh saat pakai proxy
...
def generate_persona_data(persona_type: str) -> Optional[Dict]:
    ...
    while duplicate_retry_count <= MAX_DUPLICATE_RETRIES:
        ...
        if not raw_1: logger.error(f"❌ Step 1 failed (Manual fallback exhausted)"); return None
        try:
            ...
            if duplicate_reason:
                ...
                if duplicate_retry_count > MAX_DUPLICATE_RETRIES: logger.error(f"❌ Step 1 FAILED after {MAX_DUPLICATE_RETRIES} retries (duplicates)."); return None
    ...
    if not raw_2: logger.error("❌ Step 2 failed (Manual fallback exhausted)"); return None
    try:
        ...
        if "repo_description" in asset_data and not isinstance(asset_data.get("repo_description"), (str, type(None))): logger.warning(f"⚠️ Step 2 JSON unusual: 'repo_description'. Cleaned: {cleaned_response_2[:100]}...")
        for item in asset_data.get("files", []):
            if "file_name" not in item or "file_content" not in item: logger.error(f"❌ Step 2 JSON invalid: file item keys. Cleaned: {cleaned_response_2[:100]}..."); return None
